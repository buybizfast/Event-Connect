...
import { UserPlus, MessageSquare, User, Zap, ChevronDown, ChevronUp, ArrowLeft } from 'lucide-react';
...
export default function RecommendationsPage() {
  ...
  const toggleReasons = (userId: string) => {
    if (expandedReasons === userId) {
      setExpandedReasons(null);
    } else {
      setExpandedReasons(userId);
    }
  };
                  ...
                  <div className="p-4 sm:p-6">
                    <div className="flex items-start">
                      ...
                      <div className="ml-4 flex-1">
                        ...
                        {match.allReasons.length > 1 && (
                          <button
                            onClick={() => toggleReasons(match.user.id)}
                            className="mt-1 text-xs text-indigo-600 hover:text-indigo-500 flex items-center"
                          >
                            {expandedReasons === match.user.id ? (
                              <>
                                Hide all reasons
                        ...
                        {expandedReasons === match.user.id && (
                          <div className="mt-2 text-sm text-gray-600">
                            <ul className="list-disc pl-5 space-y-1">
                              {match.allReasons.map((reason, idx) => (
